...
SELECT 
    RANK() OVER(ORDER BY movie_id ) AS movie_count,
    m.id AS movie_id, 
...
WHERE
    m.country = 'USA'
        ...
        AND (MONTH(date_published) = 3 AND YEAR(date_published) = 2017)
GROUP BY genre;
...
SELECT 
    m.title, r.avg_rating AS avg_rating, g.genre
FROM
    movie m
        INNER JOIN
...
FROM
    ...
    ratings r ON m.id = r.movie_id
WHERE
    date_published BETWEEN '2018-01-04' AND '2019-01-04'
...
SELECT 
    ...
    SUM(r.total_votes) AS sum_of_total_votes
FROM
    movie m
        INNER JOIN
    ratings r ON m.id = r.movie_id
